...
# 1. CUMPLEAÑOS
df_alumnos = pd.read_excel("Alumnos.xlsx", sheet_name="export-2")
...
# 2. DÍAS ESPECIALES
df_especiales = pd.read_excel("Menu del dia.xlsx", sheet_name="DiasEspeciales")
...
# 3. MENÚ
df_menu = pd.read_excel("Menu del dia.xlsx", sheet_name="MenuSimple")
menu = {}
...
# 4. FOTOS Y VÍDEOS
imagenes = [f"fotos/{f}" for f in os.listdir("fotos") if f.lower().endswith(('.png', '.jpg', '.jpeg', '.gif', '.mp4', '.webm', '.mov', '.avi'))]
print(f" • {len(imagenes)} archivos en galería (fotos + vídeos)")

# 5. MENSAJE PERSONALIZADO
try:
    df_mensaje = pd.read_excel("Menu del dia.xlsx", sheet_name="MensajeDia")
    mensaje_hoy = ""
    ...
    if mensaje_hoy:
        print(" • Mensaje personalizado para galería cargado")
    else:
        print(" • No hay mensaje personalizado hoy → se usa Wikipedia")
